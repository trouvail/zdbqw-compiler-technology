...
void StackMInstruction::output()
{
    if (!this->use_list.empty())
    {
        ...
        fprintf(yyout, "{");
        auto size_zhouyanlin = use_list.size();
        if (size_zhouyanlin <= 16)
        {
            this->use_list[0]->output();
            for (long unsigned int i_zhouyanlin = 1; i_zhouyanlin < use_list.size(); i_zhouyanlin++)
            {
                fprintf(yyout, ", ");
                this->use_list[i_zhouyanlin]->output();
            }
        }
        else
        {
            if (op == VPUSH)
            {
                ...
                for (long unsigned int i = 17; i < size_zhouyanlin; i++)
                {
                    fprintf(yyout, ", ");
                    this->use_list[i]->output();
                }
            ...
            {
                this->use_list[16]->output();
                for (long unsigned int i_zhouyanlin = 17; i_zhouyanlin < size_zhouyanlin; i_zhouyanlin++)
                {
                    fprintf(yyout, ", ");
                    this->use_list[i_zhouyanlin]->output();
                }
...
void MachineFunction::output()
{
    ...
    auto fp_zhouyanlin = new MachineOperand(MachineOperand::REG, 11);
    auto sp_zhouyanlin = new MachineOperand(MachineOperand::REG, 13);
    auto lr_zhouyanlin = new MachineOperand(MachineOperand::REG, 14);
    (new StackMInstruction(nullptr, StackMInstruction::PUSH, getSavedRegs(), fp_zhouyanlin, lr_zhouyanlin))->output();
    (new StackMInstruction(nullptr, StackMInstruction::VPUSH, getSavedFpRegs()))->output();
    (new MovMInstruction(nullptr, MovMInstruction::MOV, fp_zhouyanlin, sp_zhouyanlin))->output();
    // 对齐
    ...
    if (off)
    {
        ...
        if (off < -255 || off > 255)
        {
            ...
            (new LoadMInstruction(nullptr, LoadMInstruction::LDR, r4, size))->output();
            (new BinaryMInstruction(nullptr, BinaryMInstruction::SUB, sp_zhouyanlin, sp_zhouyanlin, r4))->output();
        }
        else
        {
            (new BinaryMInstruction(nullptr, BinaryMInstruction::SUB, sp_zhouyanlin, sp_zhouyanlin, size))->output();
        }
    }
    int count = 0;
...
{
    std::vector<int> constIdx_zhouyanlin;
    std::vector<int> zeroIdx_zhouyanlin;
    ...
    for (long unsigned int i = 0; i < global_list.size(); i++)
    {
        ...
        if (se->getConst())
        {
            constIdx_zhouyanlin.push_back(i);
        }
        else if (se->isAllZero())
        {
            zeroIdx_zhouyanlin.push_back(i);
        }
        else
        {
            fprintf(yyout, "\t.global %s\n", se->toStr().c_str());
    ...
    if (!constIdx_zhouyanlin.empty())
    {
        fprintf(yyout, "\t.section .rodata\n");
        for (auto i : constIdx_zhouyanlin)
        {
            IdentifierSymbolEntry *se = (IdentifierSymbolEntry *)global_list[i];
            fprintf(yyout, "\t.global %s\n", se->toStr().c_str());
            fprintf(yyout, "\t.align 4\n");
    ...
    if (!zeroIdx_zhouyanlin.empty())
    {
        for (auto i : zeroIdx_zhouyanlin)
        {
            IdentifierSymbolEntry *se = (IdentifierSymbolEntry *)global_list[i];
            if (se->getType()->isArray())
            {
...
void MachineUnit::output()
{
    ...
    fprintf(yyout, "\t.text\n");
    int count = 0;
    for (auto iter_zhouyanlin : func_list)
    {
        iter_zhouyanlin->output();
        count += iter_zhouyanlin->getSize();
        if (count > 600)
        {
            fprintf(yyout, "\tb .F%d\n", n);
            fprintf(yyout, ".LTORG\n");
...
{
    for (auto s_zhouyanlin : global_list)
    {
        IdentifierSymbolEntry *se = (IdentifierSymbolEntry *)s_zhouyanlin;
