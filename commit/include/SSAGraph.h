...
public:
    ...
    SSAGraphNode(Instruction *ins, int type = -1) : ins(ins), type(type), visited(false)
    {
        header = new SSAGraphNode();
        origin = header;
    }
    SSAGraphNode(int val) : val(val), type(CONST), visited(false)
    {
        header = new SSAGraphNode();
        origin = header;
    }
    SSAGraphNode(Operand *op, int type) : visited(false)
    {
        this->type = type;
        this->globalOperand = op;
    };
