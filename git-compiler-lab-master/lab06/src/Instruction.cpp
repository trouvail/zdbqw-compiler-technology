...
FunctionCallInstruction::FunctionCallInstruction(Operand* dst, SymbolEntry* func, std::vector<Operand*> params, BasicBlock* insert_bb) : Instruction(FUNCCALL, insert_bb)
{
    this->func = func;
    operands.push_back(dst);
    // A little puzzle
    // what if dst has been defined?
    if(dst){
        dst -> setDef(this);
    }
